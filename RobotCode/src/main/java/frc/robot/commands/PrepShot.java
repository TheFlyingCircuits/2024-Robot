 ...
 * (mainly useful for auto where the drivetrain is being controlled by
 * PathPlanner).
 * To exclude the drivetrain from this command's requirements, simply pass
 * in a null value for the translationController. The drivetrain itself can't be
 * passed
 * in as a null value, because we need to read some information from it like the
 * robot's
 ...
 * way to use this command is to run it in parallel with a
 * WaitUntil(prepShot::readyToShoot)
...
public class PrepShot extends Command {
    ...
    /**
     * Command to aim all parts of the robot at the speaker in preparation for a
     * shot.
     * This command never finishes; instead use the readyToShoot() method to
     * determine when
     * a shot should be fired. This command also provides the ability to control
     * translation
     * as the robot aims.
     * 
     * @param translationController - Supplier that provides chassisSpeeds so that
     *                              the robot can be controlled while aiming. Set
     *                              this to null when prepping a shot in auto
     *                              (this means that the drivetrain will not move at
     *                              all, only the arm and flywheels.)
     */
    public PrepShot(Drivetrain drivetrain, Arm arm, Shooter flywheels, Supplier<ChassisSpeeds> translationController,
            LEDs leds, FieldElement target) {
        ...
        if (translationController != null) {
            ...
            ledFeedbackCommand = leds.playAimingAnimationCommand(arm::getErrorDegrees, flywheels::getWorstError,
                    drivetrain::getAngleError);
        }

        // TODO: have drivetrain always on target if not required? how to deal with this
        // in auto?
    ...
    public void execute() {
        ...
        /*
         * Experiment reveals that simply averaging the requested velocities gives a
         * really solid
         * estimate for the actual exit velocity (i.e. within 1 m/s) TODO: link desmos
         * graph/experimental data.
         * This approach is preferend over actually measuring the exit velocity, because
         * we don't want the arm
         * to go nuts when the note passes through the flywheels and their speed
         * suddenly changes.
         */
        ...
        if (target == FieldElement.SPEAKER) {
            Translation3d shootOnTheMoveTarget = getShootOnTheMoveTarget(target.getLocation(), estimatedExitVelocity,
                    false);
            double armDesiredRadians = getGravCompensatedArmDesiredRadians(shootOnTheMoveTarget, estimatedExitVelocity,
                    false);
            armDesiredDegrees = Math.toDegrees(armDesiredRadians);
            driveDesiredAngle = shootOnTheMoveTarget.toTranslation2d()
                    .minus(drivetrain.getPoseMeters().getTranslation()).getAngle();
        }
        if (target == FieldElement.AMP) {
        ...
        if (target == FieldElement.LOB_TARGET) {
            ...
            driveDesiredAngle = target.getLocation().toTranslation2d()
                    .minus(drivetrain.getPoseMeters().getTranslation()).getAngle();
        ...
        if (translationController != null) {
            if (target == FieldElement.AMP) {
                drivetrain.fieldOrientedDriveOnALine(translationController.get(),
                        FieldElement.AMP.getPose().toPose2d());
            } else {
    ...
    /**
     * Calculates the angle the arm would aim at when pointing directly at the
     * target.
     */
    private double getSimpleArmDesiredDegrees(Translation3d targetLocation) {
        ...
        double radians = Math.atan2(verticalDistance, horizontalDistance); // prob don't need arctan2 here, regular
                                                                           // arctan will do.
        return Math.toDegrees(radians);
    }

    /**
     * Gets the angle that the shooter needs to aim at in order for a note to hit
     * the target
     ...
     */
    private double getGravCompensatedArmDesiredRadians(Translation3d targetLocation, double exitVelocityMetersPerSecond,
            boolean isLobShot) {
    ...
    private double getTimeToImpact(Translation3d targetLocation, double exitVelocityMetersPerSecond,
            boolean isLobShot) {
        // TODO: documentation
    ...
    private Translation3d getShootOnTheMoveTarget(Translation3d originalTargetLocation,
            double exitVelocityMetersPerSecond, boolean isLobShot) {
        // TODO: documentation
        ...
        for (int approximationCount = 0; approximationCount < 3; approximationCount += 1) {
            ...
            Translation3d robotVelocityVector = new Translation3d(robotVelocity.vxMetersPerSecond,
                    robotVelocity.vyMetersPerSecond, 0);
